...
public:
    int maxSubArray(vector<int>& nums) {            //这道题求的是连续子数组的最大和，因为符合连续和求最大值，可以使用动态规划，我原本第一次想的是采用双指针，但是结果是错的
        ...
        dp[1]=vc[1];
        for(int i=1;i<=n;i++){
            dp[i]=max(dp[i-1]+vc[i],vc[i]);           //重点：状态转移方程：当前的节点是否连接在前一个节点后面；判断标准：如果当前节点的值加上前面节点的最优值小于当前节点的值，那么放弃前面节点的值，然后自己另外起一个头；如果是前面节点的值加上当前节点的值大于当前节点的值，那么就把当前节点连接在前一个节点的后面
