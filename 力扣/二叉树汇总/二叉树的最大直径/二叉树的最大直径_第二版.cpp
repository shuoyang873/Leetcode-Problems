...
public:
    ...
    int find_diameter(TreeNode*root){        //计算每个结点两侧可以得到的最大路径长度
        ...
        int len=find_diameter(root->left)+find_diameter(root->right);     //这边计算当前结点两侧可以达到的最大长度
        max_len=max(max_len,len);          //进行比较，更新max_len
        return max(find_diameter(root->left),find_diameter(root->right))+1;    //返回在当前结点可以到达叶节点的最大长度
    }
